% !TEX encoding = UTF-8
% !TEX TS-program = pdflatex
% !TEX root = ../Tesi.tex
% !TEX spellcheck = it-IT

%*******************************************************
% Frontespizio
%*******************************************************
\begin{frontespizio}
\Preambolo{%
   \usepackage{iwona} % riga da commentare se non si carica ArsClassica
}

\Istituzione{University of Trieste}
\Logo{Sigillo}
%\Facolta{Fisica}
\Divisione{}%Department of Theoretical Physics}
%\Corso{Belle Lettere}
\Scuola{}
%\Scuola{ Classe 5A}
\Titoletto{Preliminary version}
\Titolo{Quantum Field Theory}
\Sottotitolo{Path integral formulation and renormalization}
%\Candidato[AB123456]{Lorenzo Pantieri}
\NCandidato{Author}
\Candidato{Alessandro Candolini}
%\Relatore{Francesco de Stefano}
%\Relatore{Claudio Beccari}
%\Correlatore{Tommaso Gordini}
%\Correlatore{Ivan Valbusa}
\Piede{\today}
%\Annoaccademico{2011--2012}
\end{frontespizio}





%*******************************************************
% Frontespizio alternativo
%*******************************************************
%\begin{titlepage}
%\pdfbookmark{Frontespizio}{Frontespizio}
%\changetext{}{}{}{((\paperwidth - \textwidth) / 2) - \oddsidemargin - \hoffset - 1in}{}
%\null\vfill
%\begin{center}
%\large
%\sffamily
%\bigskip

%{\LARGE\myName} \\

%\bigskip

%{\Huge\myTitle \\
%}

%\bigskip
    
%\vspace{9cm}

%\begin{tabular}{cc}
%\parbox{0.3\textwidth}{\includegraphics[width=2.5cm]{Sigillo}}
%&
%\parbox{0.7\textwidth}{{\Large\myDegree} \\ 

%					{\normalsize
%					Relatore: \myProf \\
%%					Co-relatore: \myOtherProf \\
%					
%					\myUni \\
%					\myFaculty \\
%					\myDepartment \\
%					\myTime}}
%			\end{tabular}
%\end{center}
%\vfill
%\end{titlepage}






